"""Pipeline for scraping /stats/players across all filter combinations.

Resumes automatically via progress.json. Output partitioned by year:
  data/datasets/player_stats/{year}/player_stats.parquet
"""
import asyncio
import json
import random
...
from ..modules.stats.players import (
...
_OUT_DIR    = Path(STATS_PLAYERS_DATA_DIR)
_PROGRESS_F = _OUT_DIR / STATS_PLAYERS_PROGRESS


def _year_parquet(year) -> Path:
    folder = _OUT_DIR / str(year)
    folder.mkdir(parents=True, exist_ok=True)
    return folder / STATS_PLAYERS_PARQUET
    ...
    async with new_session(headless=headless) as browser:
        ...
        for i, (year, match_type, map_name, cs_version, ranking_filter) in enumerate(remaining):
            ...
            log.info(
                ...
                i + 1, len(remaining),
                year, match_type or "all", map_name or "all",
                cs_version or "both", ranking_filter or "All",
            )
            ...
            done.add(combo_key(year, match_type, map_name, cs_version, ranking_filter))
