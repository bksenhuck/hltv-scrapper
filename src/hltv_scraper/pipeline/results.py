"""Pipeline for scraping HLTV match results.

Runners:
  run_all          — single fetch per match, saves matches + maps + player_stats (default)
  run_matches      — saves only matches.parquet
  run_maps         — re-fetches from matches.parquet URLs, saves only maps.parquet
  run_player_stats — re-fetches from matches.parquet URLs, saves only player_stats.parquet
...
from ..modules.results.maps import parse_map_rows
from ..modules.results.matches import parse_match_row
from ..modules.results.page import fetch_page_html, parse_results
from ..modules.results.player_stats import parse_stat_rows
...
from ..utils.parsers import build_results_url, extract_match_id, parse_total_pages
...
async def main() -> None:
    ...
    async with new_session(headless=not args.no_headless) as browser:
        ...
        await browser_page.goto(HLTV_BASE_URL, wait_until="domcontentloaded", timeout=PAGE_TIMEOUT_MS)
