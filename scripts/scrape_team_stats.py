"""Scrape HLTV team stats for every combination of year × event-type × map × version.
...
Output (one Parquet per year value):
  data/datasets/team_stats/all/team_stats.parquet
  data/datasets/team_stats/2012/team_stats.parquet
  data/datasets/team_stats/2013/team_stats.parquet
  ...
  data/datasets/team_stats/2026/team_stats.parquet
  data/datasets/team_stats/progress.json   — completed combo keys (resume)

Usage:
    poetry run python scripts/scrape_team_stats.py
...
OUT_DIR    = Path(STATS_TEAMS_DATA_DIR)
PROGRESS_F = OUT_DIR / STATS_TEAMS_PROGRESS


def _year_parquet(year) -> Path:
    folder = OUT_DIR / str(year)
    folder.mkdir(parents=True, exist_ok=True)
    return folder / "team_stats.parquet"
...
def _flush(buffer: list[dict], done: set[str]) -> None:
    """Write each year's rows to its own Parquet under data/datasets/team_stats/{year}/."""
    if not buffer:
        _save_progress(done)
        return
