...
Fetches results pages for the given year, selects a representative sample,
...
Usage:
    ...
    poetry run python scripts/debug_sample_matches.py --year 2012 --headless
"""
import argparse
import asyncio
import random
...
async def main(year: int, headless: bool) -> None:
    ...
    async with new_session(headless=headless) as browser:
        ...
        # --- Page 1: most recent matches ---
        html_p1 = await fetch_page_html(page, build_results_url(year=year, offset=0))
        if not html_p1:
            print(f"Failed to fetch first results page for year {year}")
            return

        total_pages = parse_total_pages(html_p1) or 1
        print(f"Year {year}: {total_pages} page(s) of results")
        ...
        for url, match in seen.items():
            ...
            print(f"  [{match.date}] match_id={match_id}  {label}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download sample match HTML pages for a given year"
    )
    parser.add_argument("--year", type=int, required=True, help="Year to sample (e.g. 2024)")
    parser.add_argument("--headless", action="store_true", help="Run browser in headless mode")
    args = parser.parse_args()
    asyncio.run(main(args.year, args.headless))
